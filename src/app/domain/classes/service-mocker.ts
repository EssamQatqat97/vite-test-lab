...
const deletePersons = async (ids: string[]) => {
  try {
    const deletionPromises = ids.map((id) =>
      axios.delete(`http://0.0.0.0:3001/persons/${id}`)
    );
...
const createPerson = async (person: TPerson) => {
  ...
  try {
    const response = await axios.post("http://0.0.0.0:3001/persons", {
      ...person,
...
const fetchPersons = async (page = 1, limit = 10) => {
  try {
    const response = await axios.get("http://0.0.0.0:3001/persons", {
...
const fetchPerson = async (id: string) => {
  try {
    const response = await axios.get(`http://0.0.0.0:3001/persons/${id}`);
